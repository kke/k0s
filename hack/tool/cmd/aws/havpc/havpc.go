...
func NewCommand() *cobra.Command {
	cmd := cobra.Command{
		Use:   "havpc",
		Short: "AWS HA (via external VPC) cluster commands",
	}
...
func newOptionsFlagSet(f *options) *pflag.FlagSet {
	...
	fs.StringVar(&f.VpcId, "vpc-id", "", "The required VPC identifier")
	fs.IntVar(&f.SubnetIdx, "subnet-idx", 0, "The index of the subnet in the VPC")
...
func buildCommand(name, desc string, runE func(cmd *cobra.Command, args []string) error) *cobra.Command {
	...
	cmd.MarkFlagRequired("subnet-idx")
...
// newCommandCreate creates a cobra.Command for creating HA k0s clusters via VPC.
func newCommandCreate() *cobra.Command {
	return buildCommand(
		"create",
		"Create an HA k0s cluster using an alternate VPC",
...
// newCommandDestroy creates a cobra.Command for destroying HA k0s clusters via VPC
func newCommandDestroy() *cobra.Command {
	return buildCommand(
		"destroy",
		"Destroy an HA k0s cluster using an alternate VPC",
		func(cmd *cobra.Command, args []string) error {
			provider := aws.Provider{}

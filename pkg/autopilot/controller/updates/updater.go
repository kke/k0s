...
func newUpdater(parentCtx context.Context, updateConfig apv1beta2.UpdateConfig, k8sClient crcli.Client, clusterID string, updateServerToken, socketPath string) (*updater, error) {
	...
	statusCtx, statusCancel := context.WithTimeout(parentCtx, 2*time.Second)
	defer statusCancel()
	status, err := status.GetStatusInfo(statusCtx, socketPath)
	if err != nil {
		return nil, err
	}

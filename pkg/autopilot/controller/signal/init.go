...
// and worker modes.
func RegisterControllers(ctx context.Context, logger *logrus.Entry, mgr crman.Manager, delegate apdel.ControllerDelegate, k0sDataDir, clusterID, socketPath string) error {
	if err := apsigk0s.RegisterControllers(ctx, logger, mgr, delegate, clusterID, socketPath); err != nil {

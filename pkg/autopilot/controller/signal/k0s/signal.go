...
func registerSignalController(logger *logrus.Entry, mgr crman.Manager, eventFilter crpred.Predicate, delegate apdel.ControllerDelegate, clusterID, statusSocket string) error {
			...
			apsigcomm.NewSignalController(
				...
				&signalControllerHandler{
					...
					k0sVersionHandler: func() (string, error) {
						return getK0sVersion(statusSocket)
					},
				},
			),
		)

...
var ErrK0sNotRunning = errors.New("k0s is not running")
...
func LoadRuntimeConfig(k0sVars *CfgVars) (*RuntimeConfigSpec, error) {
	...
	if _, err := os.FindProcess(spec.Pid); err != nil {
		// apparently this is a leftover runtime config file for a pid that does not exist, so remove it here
		// and return an error
...
func migrateLegacyRuntimeConfig(k0sVars *CfgVars, content []byte) (*RuntimeConfigSpec, error) {
	...
	return &RuntimeConfigSpec{K0sVars: k0sVars, NodeConfig: cfg}, nil
}
...
func NewRuntimeConfig(k0sVars *CfgVars) (*RuntimeConfigSpec, error) {
	if _, err := LoadRuntimeConfig(k0sVars); err == nil {
		return nil, fmt.Errorf("an instance of k0s is already running")
	}

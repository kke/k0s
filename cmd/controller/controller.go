...
func (c *command) start(ctx context.Context) error {
	...
	if errs := nodeConfig.Validate(); len(errs) > 0 {
		return fmt.Errorf("invalid node config: %w", multierr.Combine(errs...))
	}
	...
	defer func() {
		if err := rtc.Cleanup(); err != nil {
			logrus.Warnf("failed to cleanup runtime config: %v", err)
		}
	}()
	...
	if c.K0sVars.EnableDynamicConfig {
		configCtx, configCancel := context.WithTimeout(ctx, 5*time.Second)
		defer configCancel()

		clusterConfig, err := c.K0sVars.ClusterConfig(configCtx, adminClientFactory)
		if err != nil {
			logrus.Warnf("failed to load cluster config: %v", err)
		} else {
			logrus.Info("using cluster config from api")
